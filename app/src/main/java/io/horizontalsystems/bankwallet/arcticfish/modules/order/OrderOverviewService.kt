...
class OrderOverviewService(
    ...
    private val coinOverviewSubject = BehaviorSubject.create<DataState<OrderOverviewItem>>()
    val coinOverviewObservable: Observable<DataState<OrderOverviewItem>>
    ...
    private fun fetchCoinOverview() {
        marketKit.marketInfoOverviewSingle(fullCoin.coin.uid, currencyManager.baseCurrency.code, languageManager.currentLanguage)
            .subscribeIO({ marketInfoOverview ->
                coinOverviewSubject.onNext(DataState.Success(OrderOverviewItem(fullCoin.coin.code, marketInfoOverview, guideUrl)))
            }, {
